...
def create_app(registry: AgentRegistry | None = None) -> Flask:
    ...
    def create_instance(agent_id: str):
        ...
        try:
            ...
            return jsonify(
                {"instance_id": instance_id, "agent_id": agent_id, "config": config}
            ), 201
    ...
    def record_aiq(instance_id: str):
        ...
        try:
            ...
            event_id = registry.record_aiq(
                instance_id=instance_id, aiq_score=aiq_score, metrics=data.get("metrics", {})
            )

            return jsonify(
                {"event_id": event_id, "instance_id": instance_id, "aiq_score": aiq_score}
            ), 201
    ...
    def get_top_performers():
        ...
        try:
            ...
            return jsonify(
                {
                    "top_performers": [
                        {"name": name, "aiq_score": score} for name, score in performers
                    ]
                }
            )
