...
def test_merger_agent():
    ...
    try:
        ...
        if hasattr(agent, 'name') and agent.name == "MergerAgent":
            ...
            print_test("MergerAgent name property", "FAIL", f"Expected 'MergerAgent', got '{getattr(agent, 'name', 'None')}'")
            results.append(False)
...
def test_precommit_hook():
    ...
    try:
        ...
        required_elements = ["ADR-002", "100% test verification", "python run_tests.py", "exit 1", "exit 0"]
    ...
    try:
        ...
        result = subprocess.run([str(hook_path)], cwd=os.getcwd(), capture_output=True, text=True, timeout=10)
        ...
        if result.returncode in [0, 1]:
            ...
            print_test("Pre-commit hook execution", "FAIL", f"Hook exited with code {result.returncode}")
            results.append(False)
    except subprocess.TimeoutExpired:
        print_test("Pre-commit hook execution", "PASS", "Hook executed (timed out as expected for full test run)")
        results.append(True)
    except Exception as e:
...
def test_github_workflow():
    ...
    try:
        ...
        if not missing_yaml:
            ...
            print_test("GitHub workflow YAML structure", "FAIL", f"Missing elements: {missing_yaml}")
            results.append(False)
        ...
        if not missing_adr:
            ...
            print_test("GitHub workflow ADR-002 enforcement", "FAIL", f"Missing elements: {missing_adr}")
            results.append(False)
        ...
        if not missing_python:
            ...
            print_test("GitHub workflow Python setup", "FAIL", f"Missing elements: {missing_python}")
            results.append(False)
