...
class Glob(BaseTool):
    ...
    path: Optional[str] = Field(None, description="The directory to search in. If not specified, the current working directory will be used. IMPORTANT: Omit this field to use the default directory. DO NOT enter 'undefined' or 'null' - simply omit it for the default behavior. Must be a valid directory path if provided.")
        ...
        try:
            for item in os.listdir(search_path):
                if fnmatch.fnmatch(item, current_pattern):
                    ...
                    self._match_path_pattern(base_dir, remaining_patterns, new_path, matches)
        except PermissionError:
            pass
