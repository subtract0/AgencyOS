...
class TestRegistryAPIBasic:
    ...
    def test_create_agent_valid(self, client):
        ...
        response = client.post("/agents", json={"name": "TestAgent", "version": "1.0.0"})
...
class TestRegistryAPIAIQ:
    ...
    def test_record_aiq_valid(self, client):
        ...
        response = client.post(
            f"/instances/{instance_id}/aiq", json={"aiq_score": 87.5, "metrics": metrics}
        )
        assert response.status_code == 201
    ...
    def test_record_aiq_no_metrics(self, client):
        ...
        response = client.post(f"/instances/{instance_id}/aiq", json={"aiq_score": 80.0})
        assert response.status_code == 201
    ...
    def test_record_aiq_invalid_instance(self, client):
        ...
        response = client.post("/instances/invalid-id/aiq", json={"aiq_score": 75.0})
        assert response.status_code == 404
...
class TestRegistryAPIErrorHandling:
    ...
    def test_malformed_json(self, client):
        ...
        response = client.post("/agents", data="invalid json", content_type="application/json")
        assert response.status_code == 400
    ...
    def test_invalid_float_aiq_score(self, client):
        ...
        response = client.post(f"/instances/{instance_id}/aiq", json={"aiq_score": "not a number"})
