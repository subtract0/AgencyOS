...
def _fake_events() -> list[dict[str, JSONValue]]:
    return [
        ...
        },
        {
            ...
            "ts": "2025-09-25T00:10:00Z",
        },
        {
            "type": "pattern_extracted",
            "id": "p1",
            "context": {"trigger": {"type": "error"}},
            "ts": "2025-09-25T00:15:00Z",
        },
...
def test_build_cards_maps_events(monkeypatch):
    monkeypatch.setattr(
        adapters,
        "list_events",
        lambda since, grep=None, limit=None, telemetry_dir=None: _fake_events(),
    )
