...
def test_bash_python_execution():
    ...
    tool = Bash(command="python -c 'import math, json; data={\"pi\": math.pi, \"factorial_5\": math.factorial(5)}; print(json.dumps(data, indent=2))'")
    out = tool.run()
    assert "Exit code: 0" in out
...
def test_bash_environment_variables():
    ...
    tool = Bash(command="echo \"Current user: $USER, Home: $HOME, Path count: $(echo $PATH | tr ':' '\\n' | wc -l)\"")
    out = tool.run()
    assert "Exit code: 0" in out
...
def test_bash_json_processing():
    ...
    tool = Bash(command=f"echo '{json_data}' | python -c \"import json, sys; data=json.load(sys.stdin); print(f'Total users: {{data[\\\"total\\\"]}}, Average age: {{sum(u[\\\"age\\\"] for u in data[\\\"users\\\"])/len(data[\\\"users\\\"])}}')\"")
    out = tool.run()
    assert "Exit code: 0" in out
...
def test_bash_git_operations():
    ...
    with tempfile.TemporaryDirectory(dir="/tmp") as temp_dir:
        ...
        if "Exit code: 0" in out:
            ...
            tool2 = Bash(command=f"cd {temp_dir} && echo 'Hello Git' > README.md && git add README.md && git commit -m 'Initial commit' && git log --oneline")
            out2 = tool2.run()
            assert "Exit code: 0" in out2
...
def test_bash_text_processing():
    ...
    tool = Bash(command="printf 'apple\\nbanana\\napple\\ncherry\\nbanana\\napple\\n' | sort | uniq -c | sort -nr")
    out = tool.run()
    assert "Exit code: 0" in out
...
def test_bash_command_with_quotes():
    ...
    tool = Bash(command="echo \"Double quotes work\" && echo 'Single quotes work' && echo Mixed \\\"quotes\\\" work")
    out = tool.run()
...
def test_bash_mathematical_operations():
    ...
    tool = Bash(command="echo $((10 + 5 * 2)) && echo $(echo 'scale=2; 22/7' | bc -l) && python -c 'import math; print(f\"Pi: {math.pi:.6f}, E: {math.e:.6f}\")'")
    out = tool.run()
