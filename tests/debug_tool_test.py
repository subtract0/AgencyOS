...
async def test_direct_ls_tool_usage(agency):
    ...
    assert "files" in response_text.lower() or "LS" in response_text, (
        "Response should indicate file listing"
    )
...
async def test_explicit_tool_instruction(agency):
    ...
    result = await agency.get_response(
        "Execute the LS tool with path parameter set to '.' to show me the files"
    )
...
def test_manual_tool_import():
    ...
    try:
        ...
        assert "Files and directories" in ls_result or len(ls_result) > 10, (
            "LS tool should return file listing"
        )
    except ImportError:
...
def test_tools_folder_analysis():
    ...
    tool_files = [
        f for f in os.listdir(tools_path) if f.endswith(".py") and f != "__init__.py"
    ]
...
async def test_force_tool_usage(agency):
    ...
    result = await agency.get_response(
        f"I need you to run the LS tool right now to show all files in {target_dir} directory"
    )
