...
import pytest
from agency_code_agent.tools.multi_edit import MultiEdit, EditOperation
...
def test_multi_edit_atomicity_on_new_file():
    ...
    try:
        ...
        edits = [
            ...
            EditOperation(old_string="non_existent_string", new_string="replacement")  # This will fail
        ]
        ...
        assert not os.path.exists(new_file_path), f"File {new_file_path} should not exist after failed atomic operation"
...
def test_multi_edit_create_new_file(tmp_path: Path):
    ...
    edits = [
        EditOperation(old_string="", new_string="#!/usr/bin/env python3\n\ndef main():\n    print('Hello')\n    return 0"),
...
'''
    ...
    edits = [
        ...
        EditOperation(old_string="Old method implementation.", new_string="Updated method implementation."),
        EditOperation(old_string='print(f"Old: {result}")', new_string='print(f"Updated: {result}")'),
    ]
...
def test_multi_edit_json_config_update(tmp_path: Path):
  ...
  "logging": {
    ...
    edits = [
        ...
        EditOperation(old_string='"old_format"', new_string='"%(asctime)s - %(levelname)s - %(message)s"')
    ]
...
def test_multi_edit_nonexistent_file():
    ...
    edits = [
        EditOperation(old_string="old", new_string="new")
    ]
...
'''
    ...
    edits = [
        ...
        EditOperation(old_string="        # Comment with    spaces", new_string="        # Updated comment    with spaces"),
