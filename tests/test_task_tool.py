...
def test_task_short_description():
    ...
    tool = Task(
        description="Quick search", prompt="Find all TODO comments in the codebase"
    )
    result = tool.run()
...
def test_task_case_insensitive_keywords():
    ...
    tool1 = Task(
        description="SEARCH files", prompt="SEARCH for all configuration files"
    )
    result1 = tool1.run()
    ...
    tool2 = Task(
        description="Find items", prompt="Find all instances of deprecated functions"
    )
    result2 = tool2.run()
    ...
    tool3 = Task(
        description="Code review", prompt="ANALYZE the performance bottlenecks"
    )
    result3 = tool3.run()
...
def test_task_long_prompt():
    ...
    tool = Task(description="Full analysis", prompt=long_prompt)
    result = tool.run()
