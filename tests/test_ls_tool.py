...
def test_ls_with_ignore_patterns(tmp_path: Path):
    ...
    item_lines = [line for line in all_lines if ('DIR' in line or 'FILE' in line) and 'TYPE' not in line and 'Total:' not in line]
...
def test_ls_complex_directory_structure(tmp_path: Path):
    ...
    expected_items = ["README.md", "requirements.txt", "src", "tests", ".gitignore", "setup.py"]
...
def test_ls_ignore_multiple_patterns(tmp_path: Path):
    ...
    files = [
        "app.py", "app.pyc", "app.pyo",
        "test.txt", "test.bak", "test.tmp",
