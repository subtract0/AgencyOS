...
from tools.ci_monitor.cli_integration import CLIArguments, CLIExitCode, CLIIntegration
...
class TestNormalOperation:
    ...
    @patch.dict(os.environ, {"GITHUB_TOKEN": "ghp_test_token_1234"})
    @patch("tools.ci_monitor.cli_integration.autonomous_ci_fix_loop", new_callable=AsyncMock)
    async def test_invoke_with_pr_number_returns_success(
        self, mock_loop, mock_successful_loop_result, mock_agent_context
    ):
        """
    ...
    async def test_cli_argument_parsing_correct_handoff(self, mock_agent_context):
        ...
        with patch(
            "tools.ci_monitor.cli_integration.autonomous_ci_fix_loop",
