...
class LearningDashboard:
    ...
    def __init__(self, agent_context: AgentContext, vector_store: VectorStore | None = None):
        """
    ...
    def generate_comprehensive_report(self) -> dict[str, JSONValue]:
        ...
        try:
            ...
            logger.info(
                f"Generated comprehensive learning report with {len(metrics)} metrics and {len(alerts)} alerts"
            )
    ...
    def _collect_vector_store_metrics(self) -> dict[str, LearningMetric]:
        ...
        try:
            ...
            total_memories = (
                int(total_memories_raw) if isinstance(total_memories_raw, (int, float)) else 0
            )
            ...
            memories_with_embeddings = (
                int(memories_with_embeddings_raw)
                if isinstance(memories_with_embeddings_raw, (int, float))
                else 0
            )
            metrics["memories_with_embeddings"] = LearningMetric(
                ...
                trend=self._calculate_metric_trend(
                    "memories_with_embeddings", float(memories_with_embeddings)
                ),
            ...
            with_embeddings = (
                int(with_embeddings_raw) if isinstance(with_embeddings_raw, (int, float)) else 0
            )
    ...
    def _collect_pattern_application_metrics(self) -> dict[str, LearningMetric]:
        ...
        try:
            ...
            recent_applications = [
                m for m in application_memories if self._is_recent_memory(m, recent_cutoff)
            ]
            ...
            metrics["pattern_application_rate"] = LearningMetric(
                ...
                unit="per_week",
                trend=self._calculate_metric_trend(
                    "pattern_application_rate", len(recent_applications)
                ),
            ...
            successful_applications = [
                m for m in recent_applications if self._is_successful_application(m)
            ]

            success_rate = (
                (len(successful_applications) / len(recent_applications) * 100)
    ...
    def _collect_learning_progression_metrics(self) -> dict[str, LearningMetric]:
        ...
        try:
            ...
            recent_learnings = [
                m for m in learning_memories if self._is_recent_memory(m, recent_cutoff)
            ]
            ...
            metrics["learning_accumulation_rate"] = LearningMetric(
                ...
                unit="per_month",
                trend=self._calculate_metric_trend(
                    "learning_accumulation_rate", len(recent_learnings)
                ),
    ...
    def _collect_cross_session_metrics(self) -> dict[str, LearningMetric]:
        ...
        try:
            ...
            recent_cross_sessions = [
                m for m in cross_session_memories if self._is_recent_memory(m, recent_cutoff)
            ]
            ...
            metrics["cross_session_application_rate"] = LearningMetric(
                ...
                unit="per_two_weeks",
                trend=self._calculate_metric_trend(
                    "cross_session_application_rate", len(recent_cross_sessions)
                ),
    ...
    def _extract_learning_confidences(
        self, learning_memories: list[dict[str, JSONValue]]
    ) -> list[float]:
    ...
    def _calculate_knowledge_retention(
        self, cross_session_memories: list[dict[str, JSONValue]]
    ) -> float:
        """Calculate knowledge retention score."""
        try:
            ...
            successful_applications = [
                m for m in cross_session_memories if self._is_successful_application(m)
            ]
    ...
    def _count_learning_triggers(self) -> int:
        ...
        try:
            ...
            recent_triggers = [
                m for m in trigger_memories if self._is_recent_memory(m, recent_cutoff)
            ]
    ...
    def _calculate_trends(self, metrics: dict[str, LearningMetric]) -> dict[str, str]:
        ...
        try:
            ...
            vector_trends = [
                metrics[key].trend
                for key in metrics
                if key.startswith("embedding") or "memories" in key
            ]
    ...
    def _generate_learning_insights(
        self, metrics: dict[str, LearningMetric], trends: dict[str, str]
    ...
    def _calculate_system_health(
        self, metrics: dict[str, LearningMetric], alerts: list[LearningAlert]
    ) -> dict[str, JSONValue]:
    ...
    def _generate_improvement_recommendations(
        ...
        try:
            ...
            if (
                "pattern_application_rate" in metrics
                and metrics["pattern_application_rate"].value < 3
            ):
                recommendations.append(
                    {
                        "priority": "medium",
    ...
    def _create_executive_summary(
        self,
        health_score: dict[str, JSONValue],
        metrics: dict[str, LearningMetric],
        ...
        try:
            ...
            if critical_issues == 0:
                ...
                highlights.append(
                    f"{critical_issues} critical issue(s) require immediate attention"
                )
            ...
            return {
                ...
                "overall_assessment": self._get_overall_assessment(
                    float(score_val)
                    if isinstance((score_val := health_score.get("score", 0)), (int, float))
                    else 0.0,
                    critical_issues,
                ),
    ...
    def save_metrics_snapshot(self) -> str:
        ...
        try:
            ...
            self.metrics_history.append(
                {"snapshot_timestamp": datetime.now().isoformat(), "metrics": metrics_snapshot}
            )
...
def create_learning_dashboard(
    agent_context: AgentContext, vector_store: VectorStore | None = None
) -> LearningDashboard:
    """
