...
def generate_insights(patterns: JSONDict) -> list[JSONDict]:
    ...
    article_v_violations = [v for v in patterns["by_article"].items() if "Article V" in v[0]]
...
def print_report(patterns: JSONDict, insights: list[JSONDict]) -> None:
    ...
    for i, (func, count) in enumerate(
        sorted(patterns["by_function"].items(), key=lambda x: x[1], reverse=True)[:3], 1
    ):
    ...
    for insight in insights:
        if insight["type"] == "recurring_violation":
            ...
            if insight["autofix_available"]:
                print("   ✨ AutoFix available - run with --fix flag")
            print()
