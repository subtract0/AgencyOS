...
def handle_vectorstore_unavailable(
    context: AgentContext, operation: str, timeout_ms: int = 10000
...
async def handle_local_model_unavailable(
    ...
    try:
        ...
        health_result = await asyncio.wait_for(health_check_fn(), timeout=timeout_seconds)
...
def handle_precommit_failure(
    hook_output: str, repo_path: Path, auto_fix: bool = False
) -> Result[FallbackResult, FallbackError]:
    """
