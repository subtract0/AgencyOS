...
class TwoStageOrchestrator:
    ...
    async def orchestrate(
        ...
        metrics = OrchestrationMetrics(
            ...
            test_retry_count=test_results.retry_count,  # ✅ FIXED: Now tracks retries from TestVerificationGate
            patterns_used=self._aggregate_patterns_used(),  # ✅ FIXED: Aggregate from all components
            confidence_score=self._calculate_confidence(approved_spec, task_graph, test_results),  # ✅ FIXED: Calculate from component confidences
        )
    ...
    async def _validate_tests(self, task_graph: TaskGraph) -> Result[None, OrchestrationError]:
        ...
        Architectural Note:
            Full implementation requires HybridExecutor integration to extract
            test file paths from executed tasks. Current implementation validates
            test structure but defers file extraction to execution phase.

            Future enhancement (when HybridExecutor integrated):
            - Extract test files from task execution results
            - Run NECESSARY validator on actual generated test files
            - Enforce 9/9 category coverage before PR creation

            Related: tools/orchestrator/unified_primea_orchestrator.py
        """
        # Extract test task file paths (requires HybridExecutor execution context)
        # For now, skip file-level validation (structural validation done in TDDGraphGenerator)
        # Will be fully integrated when HybridExecutor execution phase is connected
    ...
    async def _execute_dag(self, task_graph: TaskGraph) -> Result[None, OrchestrationError]:
        ...
        Architectural Note:
            This method is currently stubbed pending HybridExecutor integration.
            Full implementation requires:

            1. **HybridExecutor Integration** (trinity_protocol/core/hybrid_executor.py)
               - Agent invocation via unified execution backend
               - Support for parallel task execution with dependency management
               - Result aggregation and error handling

            2. **Scheduler Integration** (tools/orchestrator/scheduler.py)
               - Topological sort for dependency-aware execution
               - Parallel execution of independent tasks
               - Progress tracking and telemetry

            3. **Task Result Extraction**
               - File paths generated by Code tasks
               - Test results from Test tasks
               - Verification targets for PR creation

            When integrated, this method will:
            - Call scheduler.execute_graph(task_graph, executor=hybrid_executor)
            - Track progress via telemetry
            - Return execution results for downstream validation

            Related Files:
            - tools/orchestrator/unified_primea_orchestrator.py (reference implementation)
            - trinity_protocol/core/hybrid_executor.py (execution backend)
            - tools/orchestrator/scheduler.py (DAG scheduler)
        ...
        Returns:
            Result with None (success) or OrchestrationError
        """
        # Stub: Full integration pending HybridExecutor connection
        # For now, assume all tasks complete successfully (enables end-to-end testing)
    ...
    async def _create_pr(
        ...
        # Collect changed files (requires HybridExecutor execution results)
        # When integrated: extract file paths from task_graph execution results
        # For now: use git diff to detect all changed files (works for testing)
        try:
            import subprocess
            ...
            files = [f for f in result_files.stdout.strip().split("\n") if f]
            if not files:
                # No changes detected, use placeholder for testing
                files = ["tools/orchestrator/two_stage_orchestrator.py"]
        except Exception as e:
            logger.warning(f"Failed to extract changed files via git diff: {e}")
            files = ["tools/orchestrator/two_stage_orchestrator.py"]  # Fallback
...
(Articles I-V enforced throughout orchestration)."""
    ...
    def _aggregate_patterns_used(self) -> int:
        """
        Aggregate VectorStore patterns used across all components.

        Returns:
            Total count of patterns applied during orchestration

        Note:
            This aggregates pattern usage from:
            - IntentParser: Backlog selection patterns
            - SpecGenerator: Spec-kit patterns
            - ApprovalCheckpoint: Slop detection patterns
            - TDDGraphGenerator: Test generation patterns
            - NECESSARYValidator: Test quality patterns
        """
        # Query VectorStore for patterns used in this session
        try:
            # Get all memories from this session with 'pattern' tag
            patterns = self.context.search_memories(
                ["pattern", "used", self.context.session_id],
                include_session=True,
            )
            return len(patterns)
        except Exception as e:
            logger.warning(f"Failed to aggregate patterns (non-blocking): {e}")
            return 0
    ...
    def _calculate_confidence(
        ...
        """
        Calculate overall workflow confidence from component confidences.

        Confidence calculation:
        - Spec quality: 0.95 if approved without edits, 0.85 with edits, 0.75 with 3+ edits
        - Test quality: 1.0 if 100% pass, 0.0 if any failures
        - Graph quality: 0.9 if all tasks have dependencies, 0.7 otherwise
        - Retry penalty: -0.1 per retry (Article I compliance indicator)

        Args:
            approved_spec: Approved specification with edit count
            task_graph: Generated task graph
            test_results: Test verification results
        ...
        spec_confidence = (
            0.95 if approved_spec.edit_count == 0
            else 0.85 if approved_spec.edit_count <= 2
            else 0.75
        )

        test_confidence = 1.0 if test_results.is_constitutional() else 0.0

        # Graph quality: check if all tasks have proper dependencies
        tasks = task_graph.all_tasks()
        tasks_with_deps = sum(1 for task in tasks if task.depends_on)
        graph_confidence = 0.9 if tasks_with_deps >= len(tasks) * 0.8 else 0.7

        # Retry penalty (indicates instability)
        retry_penalty = test_results.retry_count * 0.1

        # Weighted average with retry penalty
        confidence = (
            spec_confidence * 0.3 +
            test_confidence * 0.4 +
