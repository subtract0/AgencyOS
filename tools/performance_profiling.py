...
class PerformanceProfiler:
    ...
    def __init__(self, project_root: Path):
        ...
        self.profiler_stats: pstats.Stats | None = None

    def profile_tests(
        self, test_path: str = "tests/", pytest_args: list[str] | None = None
        ...
        try:
            ...
            result = subprocess.run(
                cmd, cwd=str(self.project_root), capture_output=True, text=True, timeout=600
            )
            ...
            if in_durations:
                ...
                if len(parts) >= 3 and parts[0].endswith("s"):
                    try:
                        ...
                        existing = next((t for t in timings if t.test_name == test_name), None)
    ...
    def _generate_report(
        self, total_duration: float, test_count: int, test_timings: list[TestTiming]
    ) -> PerformanceReport:
    ...
    def _identify_bottlenecks(self, test_timings: list[TestTiming]) -> list[Bottleneck]:
        ...
        e2e_tests = [
            t for t in test_timings if "e2e" in t.test_name.lower() or "e2e" in t.file_path.lower()
        ]
        ...
        if e2e_tests:
            total_e2e_time = sum(
                t.duration_seconds + t.setup_time + t.teardown_time for t in e2e_tests
            )
        ...
        if integration_tests:
            total_integration_time = sum(
                t.duration_seconds + t.setup_time + t.teardown_time for t in integration_tests
            )
    ...
    def _generate_recommendations(
        self, bottlenecks: list[Bottleneck], total_duration: float
    ) -> list[str]:
    ...
    def _format_markdown_report(self, report: PerformanceReport) -> str:
        ...
        for test in report.slowest_tests:
            lines.append(f"| {test['test_name']} | {test['duration']:.3f} | {test['file_path']} |")
        ...
        lines.extend(["", "---", "", "*Generated by Agency Performance Profiling System*"])

        return "\n".join(lines)

    def compare_runs(self, before: PerformanceReport, after: PerformanceReport) -> str:
...
def main() -> int:
    ...
    parser.add_argument(
        "--test-path", default="tests/", help="Path to test directory or file (default: tests/)"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--json", action="store_true", help="Output JSON format instead of markdown"
    )
