...
def get_self_healing_status(telemetry_dir: str | None = None) -> JSONValue:
    ...
    return {
        ...
        "trigger_framework_status": {"active_conditions": []},
    }
...
class MockFramework:
    def get_trigger_status(self) -> JSONValue:
        return {"triggers": {}, "active_conditions": [], "constitutional_violations": []}
...
class MockRegistry:
    def get_action_status(self) -> JSONValue:
        return {"actions": {}, "active_executions": 0, "total_executions": 0, "total_successes": 0}
    ...
    try:
        ...
        for entry in history:
            if isinstance(entry, dict):
                ...
                print(
                    f"  Action: {entry.get('action_name', 'Unknown')} ({entry.get('action_type', 'Unknown')})"
                )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="agency self-healing", description="Self-healing trigger management and monitoring"
    )

    # Global options
    parser.add_argument("--format", choices=["text", "json"], default="text", help="Output format")
    parser.add_argument("--telemetry-dir", help="Custom telemetry directory path")
    ...
    status_parser.add_argument(
        "--health", action="store_true", help="Show health check instead of full status"
    )
    ...
    history_parser.add_argument(
        "--limit", type=int, default=20, help="Maximum number of entries to show"
    )
