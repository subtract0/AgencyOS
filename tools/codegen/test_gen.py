...
def generate_tests_from_spec(spec_path: str, out_dir: str) -> list[GeneratedTest]:
    ...
    try:
        ...
        if not criteria:
            return [GeneratedTest(path=spec_path, ac_id="", name="", status="error")]
        ...
        for ac_id, description in criteria.items():
            ...
            try:
                ...
                results.append(
                    GeneratedTest(path=test_file, ac_id=ac_id, name=test_name, status="created")
                )
            except OSError as e:
                results.append(
                    GeneratedTest(path=test_file, ac_id=ac_id, name=test_name, status=f"error: {e}")
                )

    except OSError as e:
        results.append(
            GeneratedTest(
                path=spec_path, ac_id="", name="", status=f"error: Could not read spec file: {e}"
            )
        )
...
def _parse_acceptance_criteria(content: str) -> dict[str, str]:
    ...
    for line in lines:
        ...
        if (
            in_ac_section
            and line.startswith("#")
            and not re.match(r"^#+\s*acceptance\s+criteria", line, re.IGNORECASE)
        ):
            break
