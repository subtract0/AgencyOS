...
async def main():
    ...
    witness = WitnessAgent(message_bus=message_bus, pattern_store=pattern_store)

    architect = ArchitectAgent(message_bus=message_bus, pattern_store=pattern_store)

    executor = ExecutorAgent(
        message_bus=message_bus, cost_tracker=cost_tracker, agent_context=agent_context
    )
    ...
    print("  ✅ WITNESS → ARCHITECT → EXECUTOR flow operational")
    print(f"  ✅ {wired_count}/6 sub-agents wired (CODE, TEST, TOOL, QUALITY, MERGE, SUMMARY)")
    print("  ✅ MessageBus: inter-agent communication")
    print("  ✅ PatternStore: persistent pattern storage")
