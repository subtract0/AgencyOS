...
class QAQuestion(BaseModel):
    ...
    question_text: str = Field(
        ..., min_length=10, max_length=500, description="Question text (10-500 chars)"
    )
    question_number: int = Field(..., ge=1, le=10, description="Question number in sequence (1-10)")
    required: bool = Field(..., description="Is answer required before proceeding?")
...
class QAAnswer(BaseModel):
    ...
    question_id: str = Field(..., description="Links answer to question")
    answer_text: str = Field(
        ..., min_length=1, max_length=2000, description="User's answer (1-2000 chars)"
    )
...
class QASession(BaseModel):
    ...
    total_time_minutes: int | None = Field(
        default=None, ge=0, description="Total time spent in session"
    )
...
class AcceptanceCriterion(BaseModel):
    ...
    description: str = Field(..., min_length=10, max_length=500, description="Verifiable criterion")
...
class ProjectTask(BaseModel):
    ...
    estimated_minutes: int = Field(..., ge=1, le=60, description="Estimated duration (1-60 min)")
...
class ProjectPlan(BaseModel):
    ...
    timeline_start: datetime = Field(default_factory=datetime.now, description="Plan start date")
    timeline_end_estimate: datetime = Field(..., description="Estimated completion date")
...
class CheckinQuestion(BaseModel):
    ...
    project_id: str = Field(..., description="Associated project")
    task_id: str | None = Field(default=None, description="Related task if applicable")
...
class CheckinResponse(BaseModel):
    ...
    response_text: str = Field(..., min_length=1, max_length=2000, description="User's response")
...
class DailyCheckin(BaseModel):
    ...
    next_steps: str = Field(default="", max_length=500, description="What system will do next")
...
class ProjectMetadata(BaseModel):
    ...
    daily_time_commitment_minutes: int = Field(
        ..., ge=5, le=60, description="Daily time commitment (5-60 min)"
    )
...
class Project(BaseModel):
    ...
    state: ProjectState = Field(
        default=ProjectState.INITIALIZING, description="Current project state"
    )
...
class ProjectOutcome(BaseModel):
    ...
    completion_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Completion percentage (0.0-1.0)"
    )
    total_time_minutes: int = Field(..., ge=0, description="Total time invested")
    total_checkins: int = Field(..., ge=0, description="Number of check-ins conducted")
    user_satisfaction: int | None = Field(
        default=None, ge=1, le=5, description="User satisfaction rating (1-5)"
    )
    deliverable_quality: int | None = Field(
