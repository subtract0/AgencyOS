...
class AudioConfig(BaseModel):
    ...
    sample_rate: int = Field(
        default=16000, ge=8000, le=48000, description="Sample rate in Hz (Whisper prefers 16kHz)"
    )
    channels: int = Field(
        default=1, ge=1, le=2, description="Number of audio channels (1=mono, 2=stereo)"
    )
    chunk_size: int = Field(
        default=1024, ge=512, le=8192, description="Audio buffer chunk size in frames"
    )
    buffer_seconds: float = Field(
        default=30.0, gt=0.0, le=120.0, description="Max buffer duration before flush"
    )
...
class WhisperConfig(BaseModel):
    ...
    model_path: str = Field(..., description="Path to Whisper GGML model file")
    ...
    use_gpu: bool = Field(default=True, description="Enable Metal GPU acceleration (M4 Pro)")
    num_threads: int = Field(default=4, ge=1, le=16, description="CPU threads for inference")
    beam_size: int = Field(
        default=5, ge=1, le=10, description="Beam search size (higher=more accurate, slower)"
    )
    # Improvement #1: Whisper accuracy parameters
    temperature: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Sampling temperature (0.0=deterministic)"
    )
    compression_ratio_threshold: float = Field(
        default=2.4, gt=0.0, description="Hallucination detection threshold"
    )
    logprob_threshold: float = Field(
        default=-1.0, le=0.0, description="Low-confidence token filtering threshold"
    )
...
class TranscriptionResult(BaseModel):
    ...
    timestamp: str = Field(..., description="ISO8601 timestamp of transcription")
...
class AudioCaptureStats(BaseModel):
    ...
    total_duration_seconds: float = Field(default=0.0, ge=0.0, description="Total audio captured")
    speech_duration_seconds: float = Field(
        default=0.0, ge=0.0, description="Duration with detected speech"
    )
    silence_duration_seconds: float = Field(default=0.0, ge=0.0, description="Duration of silence")
    segments_captured: int = Field(default=0, ge=0, description="Number of audio segments")
    segments_transcribed: int = Field(
        default=0, ge=0, description="Number of segments sent to Whisper"
    )
