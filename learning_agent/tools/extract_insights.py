...
class ExtractInsights(BaseTool):  # mypy: disable-error-code="misc"
    ...
    confidence_threshold: float = Field(
        default=0.7, description="Minimum confidence threshold for insights (0.0 to 1.0)"
    )
    ...
    def _extract_tool_insights(self, analysis_data: dict[str, JSONValue]) -> list[ExtractedInsight]:
        ...
        if low_success_tools:
            ...
            low_success_insight = ExtractedInsight(
                ...
                confidence=0.85,
                data={
                    "low_success_tools": cast(
                        list[JSONValue], [(t, r) for t, r in low_success_tools]
                    )
                },
    ...
    def _extract_error_insights(
        self, analysis_data: dict[str, JSONValue]
        ...
        # Resolution patterns
        resolution_patterns = cast(
            list[dict[str, JSONValue]], error_analysis.get("resolution_patterns", [])
        )
        if resolution_patterns:
            avg_resolution_steps = sum(
                cast(int, p.get("steps_to_resolution", 0)) for p in resolution_patterns
            ...
            insights.append(
                ExtractedInsight(
                    ...
                    data={
                        ...
                        "avg_steps": avg_resolution_steps,
                    },
    ...
    def _extract_workflow_insights(
        self, analysis_data: dict[str, JSONValue]
    ) -> list[ExtractedInsight]:
        ...
        if avg_length > 0:
            if avg_length < 5:
                ...
                efficiency_insight = (
                    "Workflows may be too complex - consider breaking into smaller tasks"
                )
                confidence = 0.85
            else:
    ...
    def _extract_optimization_insights(
        ...
        interaction_analysis = cast(
            dict[str, JSONValue], analysis_data.get("interaction_analysis", {})
        )
