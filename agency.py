...
def _cli_event_scope(command: Optional[str]=None, args_dict: Optional[Dict[str, JSONValue]]=None):
    ...
    try:
        ...
        try:
            finished = time.time()
            _tel_emit({'type': 'cli_command_finished', 'command': command, 'status': 'success', 'finished_at': finished, 'duration_s': max(0.0, finished - started)})
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
        ...
        try:
            finished = time.time()
            _tel_emit({'type': 'cli_command_finished', 'command': command, 'status': 'failed', 'error': str(e), 'finished_at': finished, 'duration_s': max(0.0, finished - started)})
...
from agency_memory import Memory, create_firestore_store, EnhancedMemoryStore, create_enhanced_memory_store
...
planner = create_planner_agent(model=agent_model('planner'), reasoning_effort='high', agent_context=shared_context)
coder = create_agency_code_agent(model=agent_model('coder'), reasoning_effort='medium', agent_context=shared_context)
auditor = create_auditor_agent(model=agent_model('auditor'), reasoning_effort='high', agent_context=shared_context)
test_generator = create_test_generator_agent(model=agent_model('test_generator'), reasoning_effort='medium', agent_context=shared_context)
learning_agent = create_learning_agent(model=agent_model('learning'), reasoning_effort='high', agent_context=shared_context)
chief_architect = create_chief_architect_agent(model=agent_model('chief_architect'), reasoning_effort='medium', agent_context=shared_context)
merger = create_merger_agent(model=agent_model('merger'), reasoning_effort='medium', agent_context=shared_context)
summary = create_work_completion_summary_agent(model=agent_model('summary'), reasoning_effort='low', agent_context=shared_context)
toolsmith = create_toolsmith_agent(model=agent_model('toolsmith'), reasoning_effort='medium', agent_context=shared_context)
quality_enforcer = create_quality_enforcer_agent(model=agent_model('quality_enforcer'), reasoning_effort='high', agent_context=shared_context)
agency = Agency(chief_architect, coder, planner, auditor, test_generator, learning_agent, merger, summary, toolsmith, quality_enforcer, name='AgencyCode', communication_flows=[(chief_architect, auditor, SendMessageHandoff), (chief_architect, learning_agent, SendMessageHandoff), (chief_architect, planner, SendMessageHandoff), (chief_architect, toolsmith, SendMessageHandoff), (chief_architect, quality_enforcer, SendMessageHandoff), (planner, coder, SendMessageHandoff), (coder, planner, SendMessageHandoff), (planner, auditor, SendMessageHandoff), (auditor, test_generator, SendMessageHandoff), (auditor, quality_enforcer, SendMessageHandoff), (test_generator, coder, SendMessageHandoff), (quality_enforcer, coder, SendMessageHandoff), (quality_enforcer, test_generator, SendMessageHandoff), (coder, merger, SendMessageHandoff), (toolsmith, merger, SendMessageHandoff), (coder, summary, SendMessageHandoff), (planner, summary, SendMessageHandoff), (merger, summary, SendMessageHandoff)], shared_instructions='./project-overview.md')
import sys
import argparse
...
def _render_dashboard_text(summary: Dict[str, JSONValue]) -> None:
    ...
    if running:
        ...
        print('- none')
    print(f"Recent Results: success={recent.get('success', 0)} failed={recent.get('failed', 0)} timeout={recent.get('timeout', 0)}")
    ...
    print(f"Window: since={window.get('since')} events={total} started={metrics.get('tasks_started', 0)} finished={metrics.get('tasks_finished', 0)}")
...
def _list_recent_files(directory: str, description: str, limit: int=5) -> None:
    ...
    if os.path.isdir(directory):
        try:
            entries = sorted(((name, os.path.getmtime(os.path.join(directory, name))) for name in os.listdir(directory)), key=lambda x: x[1], reverse=True)[:limit]
...
def _check_test_status() -> None:
    ...
    print('  Test Status:')
    try:
        result = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_memory_api.py', '-q'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=25)
...
def _check_recent_activity() -> None:
    ...
    try:
        ...
        if count:
            ...
            print(f'    📊 No recent autonomous healing activity')
    except Exception:
...
def _cmd_cost_dashboard(args: argparse.Namespace) -> None:
    ...
    with _cli_event_scope('cost_dashboard', {'live': args.live}):
        ...
        dashboard = CostDashboard(cost_tracker=tracker, refresh_interval=args.interval, budget_warning_pct=args.warning_pct)
        if args.live:
