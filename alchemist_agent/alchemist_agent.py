...
def create_alchemist_agent(
    model: str = "gpt-5",
    reasoning_effort: str = "medium",
    agent_context: AgentContext | None = None,
) -> Agent:
    """Factory that returns a fresh Alchemist agent instance.
    ...
    agent_context.store_memory(
        f"alchemist_agent_created_{agent_context.session_id}",
        {
    ...
    return Agent(
        name="Alchemist",
        description=(
            "Pain-to-hook converter that distills raw user language into high-performing marketing headlines. "
            "Extracts underlying tensions and desired outcomes, then delivers JSON-only output for downstream systems."
        ),
