...
def parse_rec(path: Path) -> dict | None:
    ...
    try:
        c = path.read_text()
        p, cat, e, s = (re.search(r"\*\*Priority\*\*:\s*(\w+)", c), re.search(r"\*\*Category\*\*:\s*(\w+)", c),
                        re.search(r"\*\*Effort\*\*:\s*([\d.]+)", c), re.search(r"\*\*Status\*\*:\s*(\w+)", c))
        if not all([p, cat, e, s]):
            return None
        return {"id": path.stem, "priority": p.group(1), "category": cat.group(1),
                "summary": (m.group(1).strip()[:50] + "..." if (m := re.search(r"## Summary\n(.+?)(?:\n\n|\n##)", c, re.DOTALL)) else "No summary"),
                "files_count": len(re.findall(r"-\s*`([^`]+?)`", c)), "effort": float(e.group(1)), "status": s.group(1)}
    except Exception:
        return None
...
def query_conf(ctx, recs: list[dict]) -> dict[str, float]:
    ...
    conf = {}
    for r in recs:
        pats = ctx.search_memories(tags=["pattern", "fix", r["category"].lower()], include_session=False)
        conf[r["id"]] = min(sum(p.get("confidence", 0.7) for p in pats) / len(pats) + 0.1, 1.0) if pats else {"P3": 0.9, "P2": 0.7, "P1": 0.6, "P0": 0.4}.get(r["priority"], 0.7)
    return conf
...
def gen_dash(recs: list[dict], stats: dict, state: dict, conf: dict, out: Path) -> None:
    ...
    lines = [f"# Autonomous Fix Recommendations - Human Review Dashboard\n",
             f"**Generated**: {now}", f"**Total**: {stats['total']}", f"**Session**: {state.get('session_id', 'N/A')}\n", "---\n",
             "## 1. Summary Statistics\n", "### Status Breakdown\n",
             f"- **Pending**: {stats['total'] - state.get('processed', 0)}", f"- **Applied**: {state.get('applied', 0)}",
             f"- **Failed**: {state.get('failed', 0)}", f"- **Skipped**: {state.get('skipped', 0)}\n",
             "### Priority Breakdown\n", f"- **P3** (Low): {stats['p3']}", f"- **P2** (Medium): {stats['p2']}",
             f"- **P1** (High): {stats['p1']}", f"- **P0** (Critical): {stats['p0']}\n", "### Category Breakdown\n"]
    ...
    lines.extend(["\n### Success Metrics\n", f"- **Success Rate**: {stats['success_rate']:.1f}%",
                  ...
                  "## 2. Quick Actions\n", "```bash",
                  "# Approve all P3 (safest)", "python scripts/autonomous_recommendation_fixer.py --priority P3 --batch-all\n",
                  "# Review P1", "python scripts/autonomous_recommendation_fixer.py --priority P1 --dry-run\n",
                  "# Process category", "python scripts/autonomous_recommendation_fixer.py --category pruning --limit 50",
                  "```\n", "---\n", "## 3. Recommendation Table\n",
                  "| ID | Priority | Category | Summary | Files | Effort (h) | Confidence |",
                  "|:---|:---------|:---------|:--------|------:|-----------:|:-----------|"])
    sorted_recs = sorted(recs, key=lambda r: ({"P0": 0, "P1": 1, "P2": 2, "P3": 3}.get(r["priority"], 4), -conf.get(r["id"], 0.7)))
    for rec in sorted_recs[:100]:
        c = conf.get(rec["id"], 0.7)
        c_d = f"🟢 {c:.2f}" if c > 0.8 else f"🟡 {c:.2f}" if c > 0.6 else f"🔴 {c:.2f}"
        lines.append(f"| {rec['id'][:30]} | {rec['priority']} | {rec['category']} | {rec['summary'][:35]} | {rec['files_count']} | {rec['effort']:.1f} | {c_d} |")
    ...
    if results:
        lines.extend(["| Time | Category | Status | Files | Tests | Commit |", "|:-----|:---------|:-------|------:|:------|:-------|"])
        for res in results[-20:]:
            rd = res.get("recommendation", {})
            emoji = {"applied": "✅", "failed": "❌", "skipped": "⏭️"}.get(res.get("status", "").lower(), "❓")
            commit = res.get("commit_sha", "N/A")[:8] if res.get("commit_sha") else "N/A"
            lines.append(f"| {now[11:16]} | {rd.get('category', 'N/A')} | {emoji} {res.get('status', 'N/A')} | {len(res.get('files_modified', []))} | {'✅' if res.get('tests_passed') else '❌'} | {commit} |")
    else:
...
def main() -> int:
    ...
    parser.add_argument("--recommendations-dir", type=Path, default=Path(".output/audit_recommendations"))
    parser.add_argument("--state-file", type=Path, default=Path(".fixer_state.json"))
    parser.add_argument("--output", type=Path, default=Path(".output/autonomous_fixes/review_dashboard.md"))
    parser.add_argument("--category", type=str)
    args = parser.parse_args()

    print(f"Generating dashboard...\n  Recommendations: {args.recommendations_dir}\n  Output: {args.output}")
    state = load_state(args.state_file)
    recs = [rec for f in sorted(args.recommendations_dir.glob("*.md")) if (rec := parse_rec(f)) and (not args.category or rec["category"].lower() == args.category.lower())]
    if not recs:
