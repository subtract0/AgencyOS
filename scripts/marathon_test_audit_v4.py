#!/usr/bin/env python3
"""
MARATHON TEST AUDIT V4 - Calibrated with V3 Validation Learnings

Improvements over V3:
1. ✅ Ground truth-aligned recalibration (V3: 41% accuracy → V4 target: ≥85%)
2. ✅ Focused tests: Ignore ALL non-focal gaps (V3 issue: P2 for non-applicable gaps)
3. ✅ General tests: Stricter threshold (1+ missing core → P1, was 2+ in V3)
4. ✅ Expected: P1 alignment with Sonnet 4.5 ground truth distribution

Improvements over V2:
1. ✅ Test purpose classification (focused vs general tests)
2. ✅ Purpose-aware recalibration (lenient for focused, strict for general)
3. ✅ Sonnet 4.5 ground truth intelligence (same logic I use)

Usage:
    # Quick test (100 tests)
    python scripts/marathon_test_audit_v4.py --max-tests 100

    # Full audit
    python scripts/marathon_test_audit_v4.py --depth standard
"""
...
class TestAnalysis:
    ...
    test_purpose: str = "general"  # NEW V3: focused_security, focused_validation, general, etc.
...
class ImprovedAuditor:
    ...
    def __init__(self, depth: str = "standard", max_tests: Optional[int] = None, suggestions: bool = False):
        ...
        self.state_file = Path(".marathon_audit_v4_state.json")
    ...
    def classify_test_purpose(self, test_name: str, test_code: str) -> str:
        """
        NEW V3: Classify test purpose using Sonnet 4.5 ground truth intelligence.

        This is the SAME logic I used to establish ground truth.
        Focused tests test ONE aspect; general tests cover comprehensively.
        """
        # Security-focused (command injection, path traversal, etc.)
        security_keywords = [
    ...
    def recalibrate_priority(self, test_analysis: TestAnalysis) -> str:
        """
        NEW V4 FEATURE: Calibrated recalibration using V3 validation learnings.

        V1 Problem: 85% P1 (too many false positives)
        V2 Problem: 45% P1 (still too high - focused tests over-reported)
        V3 Problem: 25% P1 (too lenient - validation showed 41% accuracy)
        V4 Solution: Match Sonnet 4.5 ground truth thresholds
        ...
        Priority Logic:
        - Focused tests: P1 ONLY if missing focal category, else P3
        - General tests: P1 if missing 1+ core categories
        """
        ...
        test_purpose = test_analysis.test_purpose
    ...
    def analyze_test_function(self, test_file: Path, test_name: str, start_line: int, end_line: int) -> TestAnalysis:
        ...
        # V3: Classify test purpose using ground truth intelligence
        test_purpose = self.classify_test_purpose(test_name, test_code)

        applicable_categories = self.get_applicable_categories(test_code, test_type)

        # V3: Enhanced prompt with test purpose context
...
"""
        ...
        analysis = TestAnalysis(
            ...
            test_type=test_type,
            test_purpose=test_purpose  # V3: Ground truth classification
        )

        # V3: Recalibrate priority using test purpose
    ...
    def run_audit(self):
        ...
        print("="*80)
        print("🚀 MARATHON TEST AUDIT V4 - Calibrated with V3 Validation Learnings")
        print("="*80)
        print(f"Model: {MODEL}")
        print(f"Depth: {self.depth}")
        print(f"Max Tests: {self.max_tests or 'ALL'}")
        print()
        print("V4 Improvements (calibrated with V3 validation):")
        print("  ✅ Focused tests: Ignore ALL non-focal gaps")
        print("  ✅ General tests: Stricter threshold (1+ missing core → P1)")
        print("  ✅ Expected: ≥85% priority accuracy (V3: 41%)")
        print()
    ...
    def generate_reports(self):
        ...
        # JSON report
        json_path = self.output_dir / f"marathon_audit_v4_{timestamp}.json"
        with open(json_path, 'w') as f:
            json.dump([asdict(r) for r in self.results], f, indent=2)
        print(f"  ✅ JSON: {json_path}")

        # Markdown report
        md_path = self.output_dir / f"marathon_audit_v4_{timestamp}.md"
        self._generate_markdown_report(md_path)
        print(f"  ✅ Markdown: {md_path}")

        # Healing roadmap
        roadmap_path = self.output_dir / f"healing_roadmap_v4_{timestamp}.md"
