#!/usr/bin/env python3
"""
MARATHON TEST AUDIT V3 - Ground Truth Intelligence
...
4. ✅ Expected: P1 reduction from 45% → 12-15%

Improvements over V1:
1. ✅ Applicability filter (reduces false gaps 60% → 2%)
2. ✅ Recalibrated priority system (reduces P1 91% → 12-15%)
3. ✅ Confidence scoring per issue

Usage:
    # Quick test (100 tests)
    python scripts/marathon_test_audit_v3.py --max-tests 100
...
class ImprovedAuditor:
    ...
    def __init__(self, depth: str = "standard", max_tests: Optional[int] = None, suggestions: bool = False):
        ...
        self.state_file = Path(".marathon_audit_v3_state.json")
    ...
    def recalibrate_priority(self, test_analysis: TestAnalysis) -> str:
        """
        NEW V3 FEATURE: Test purpose-aware recalibration using Sonnet 4.5 ground truth intelligence.

        V1 Problem: 85% P1
        V2 Problem: 45% P1 (still too high - focused tests over-reported)
        V3 Solution: Lenient for focused tests, strict for general tests

        Priority Logic:
        - Focused tests: Only P1 if missing FOCAL category
        - General tests: P1 if missing 2+ core categories
        """
        ...
        # V3: Purpose-aware logic
        test_purpose = test_analysis.test_purpose

        if test_purpose.startswith('focused_'):
            # Focused tests: LENIENT (only flag if missing focal category)
            focal_map = {
            ...
            # P2: Missing secondary categories (e.g., validation missing Edge cases)
            ...
            # P2: Missing 1 core or any categories
            if len(applicable_gaps) > 0:
                return 'P2'
...
"""
    ...
    def run_audit(self):
        ...
        print("="*80)
        print("🚀 MARATHON TEST AUDIT V3 - Ground Truth Intelligence")
        print("="*80)
        print(f"Model: {MODEL}")
        print(f"Depth: {self.depth}")
        print(f"Max Tests: {self.max_tests or 'ALL'}")
        print()
        print("V3 Improvements (using Sonnet 4.5 intelligence):")
        print("  ✅ Test purpose classification (focused vs general)")
        print("  ✅ Purpose-aware priorities (lenient for focused tests)")
        print("  ✅ Expected: P1 reduction 45% → 12-15%")
        print()
    ...
    def generate_reports(self):
        ...
        # JSON report
        json_path = self.output_dir / f"marathon_audit_v3_{timestamp}.json"
        with open(json_path, 'w') as f:
            json.dump([asdict(r) for r in self.results], f, indent=2)
        print(f"  ✅ JSON: {json_path}")

        # Markdown report
        md_path = self.output_dir / f"marathon_audit_v3_{timestamp}.md"
        self._generate_markdown_report(md_path)
        print(f"  ✅ Markdown: {md_path}")

        # Healing roadmap
        roadmap_path = self.output_dir / f"healing_roadmap_v3_{timestamp}.md"
