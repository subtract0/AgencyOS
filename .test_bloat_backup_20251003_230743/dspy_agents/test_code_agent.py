...
def sample_file_changes():
    ...
    return [
        ...
        FileChange(file_path="/test/file2.py", operation="create", content="# New file content"),
    ]
    ...
    def test_factory_function_creates_agent(self):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            agent = create_dspy_code_agent(
                model="gpt-4", reasoning_effort="low", enable_learning=False, quality_threshold=0.7
            )
...
class TestDSPyCodeAgentForwardMethod:
    ...
    def test_forward_with_dspy_available_success(
        self, mock_dspy_module, sample_context, sample_agent_result
    ):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            result = agent.forward(
                task_description="Create a test function", context=sample_context
            )
    ...
    def test_forward_with_minimal_context(self):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            result = agent.forward(task_description="Simple task", context={})
    ...
    def test_forward_enables_learning_on_success(
        self, mock_dspy_module, sample_context, sample_agent_result
    ):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            # Execute the forward method
            result = agent.forward(
                task_description="Create a new authentication feature", context=sample_context
            )
            ...
            )  # "create" keyword triggers implementation
...
class TestDSPyCodeAgentPlanningMethods:
    ...
    def test_plan_task_with_dspy_available(
        self, mock_dspy_module, sample_context, sample_task_plan
    ):
    ...
    def test_plan_task_with_dspy_unavailable_fallback(self, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", False):
            ...
            # Execute planning
            plan = agent.plan_task(task="Create user authentication", context=sample_context)
    ...
    def test_plan_task_classifies_testing_tasks(self, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", False):
            ...
            # Plan a testing task
            plan = agent.plan_task(task="Write comprehensive test suite", context=sample_context)
...
class TestDSPyCodeAgentImplementationMethods:
    ...
    def test_implement_plan_with_dspy_available(
        self,
        mock_dspy_module,
        sample_context,
        sample_task_plan,
    ...
    def test_implement_plan_with_dspy_unavailable_fallback(self, sample_task_plan, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", False):
            ...
            # Execute implementation
            changes, tests, notes = agent.implement_plan(
                plan=sample_task_plan, context=sample_context
            )
    ...
    def test_implement_plan_handles_implementation_error(
        self, mock_dspy_module, sample_task_plan, sample_context
    ):
...
class TestDSPyCodeAgentVerificationMethods:
    ...
    def test_verify_implementation_with_dspy_unavailable_fallback(self, sample_agent_result):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", False):
            ...
            result = agent.verify_implementation(
                implementation=sample_agent_result, test_results=test_results
            )
    ...
    def test_verify_implementation_handles_verification_error(
        self, mock_dspy_module, sample_agent_result
    ):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            # Execute verification
            result = agent.verify_implementation(
                implementation=sample_agent_result, test_results={}
            )
...
class TestDSPyCodeAgentLearningSystem:
    ...
    def test_get_learning_summary(self, sample_context, sample_agent_result):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            agent._learn_from_execution(
                "Success task", sample_agent_result, agent._prepare_context(sample_context)
            )

            failed_result = AgentResult(
                success=False, changes=[], tests=[], verification=None, message="Failed"
            )
            agent._learn_from_execution(
                "Failed task", failed_result, agent._prepare_context(sample_context)
            )
    ...
    def test_reset_learning(self, sample_context, sample_agent_result):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            # Add some patterns
            agent._learn_from_execution(
                "Task", sample_agent_result, agent._prepare_context(sample_context)
            )
...
class TestDSPyCodeAgentEdgeCases:
    ...
    def test_learning_with_disabled_learning(
        self, mock_dspy_module, sample_context, sample_agent_result
    ):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            result = agent.forward(task_description="Test task", context=sample_context)
...
class TestDSPyCodeAgentConstitutionalCompliance:
    ...
    def test_constitutional_articles_are_included_in_context(self, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            assert (
                ...
                in context.constitutional_articles
            )
            assert (
    ...
    def test_constitutional_constraints_added_to_planning(self, mock_dspy_module, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            # Execute planning
            agent.plan_task(
                task="Create feature", context=sample_context, constraints=["Custom constraint"]
            )
    ...
    def test_constitutional_compliance_tracked_in_learning(
        self, sample_context, sample_agent_result
    ):
...
class TestDSPyCodeAgentIntegration:
    ...
    def test_full_workflow_with_dspy_available(self, mock_dspy_module, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            result = agent.forward(
                task_description="Create a new feature with tests", context=sample_context
            )
    ...
    def test_full_workflow_with_dspy_unavailable(self, sample_context):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", False):
            ...
            # Execute workflow
            result = agent.forward(task_description="Create a new feature", context=sample_context)
    ...
    def test_workflow_with_planning_implementation_verification(
        self, mock_dspy_module, sample_context
    ):
        ...
        with patch("dspy_agents.modules.code_agent.DSPY_AVAILABLE", True):
            ...
            implementation = AgentResult(
                success=True, changes=changes, tests=tests, verification=None, message="Test"
            )
