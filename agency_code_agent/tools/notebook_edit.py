...
class NotebookEdit(BaseTool):
    ...
    notebook_path: str = Field(..., description="The absolute path to the Jupyter notebook file to edit (must be absolute, not relative)")
    cell_id: Optional[str] = Field(None, description="The ID of the cell to edit. When inserting a new cell, the new cell will be inserted after the cell with this ID, or at the beginning if not specified.")
    new_source: str = Field(..., description="The new source for the cell")
    cell_type: Optional[Literal["code", "markdown"]] = Field(None, description="The type of the cell (code or markdown). If not specified, it defaults to the current cell type. If using edit_mode=insert, this is required.")
    edit_mode: Optional[Literal["replace", "insert", "delete"]] = Field("replace", description="The type of edit to make (replace, insert, delete). Defaults to replace.")
