...
class MilestoneMetrics(BaseModel):
    ...
    total_tasks: int = Field(
        ...,
        ge=0,
        description="Total tasks processed since monitoring start"
    )
    tasks_since_last_milestone: int = Field(
        ...,
        ge=0,
        description="Tasks processed since previous milestone (or start)"
    )
    ...
    overall_accuracy: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Cumulative routing accuracy (correct / total)"
    )
    interval_accuracy: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Accuracy since last milestone (improvement tracking)"
    )
    ...
    misclassifications_detected: int = Field(
        0,
        ge=0,
        description="Total misclassifications detected (CRITICAL/WARNING)"
    )
    detection_rate: float = Field(
        0.0,
        ge=0.0,
        le=1.0,
        description="Ratio of misclassifications to total tasks"
    )

    # Refinement metrics
    refinements_applied: int = Field(
        0,
        ge=0,
        description="Total VectorStore refinements applied"
    )
    ...
    avg_refinement_confidence: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Average confidence of applied refinements"
    )
    ...
    p3_accuracy: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Accuracy for P3 (simple) tasks"
    )
...
class MonitoringMilestone(BaseModel):
    ...
    milestone_number: int = Field(
        ...
        ge=1,
        le=4,
        description="Milestone number (1=25 tasks, 2=50, 3=75, 4=100)"
    )
    ...
    tasks_processed: int = Field(
        ...,
        ge=0,
        description="Actual tasks processed (may exceed threshold)"
    )

    # Metrics snapshot
    metrics: MilestoneMetrics = Field(
        ...,
        description="Aggregated metrics at milestone"
    )
