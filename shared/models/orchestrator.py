...
class ExecutionMetrics(BaseModel):
    ...
    additional: dict[str, JSONValue] = Field(
        default_factory=dict, description="Extra metrics keyed by name"
    )
...
class TaskResultModel(BaseModel):
    ...
    status: Literal["success", "failed", "canceled", "timeout"] = Field(
    ...
    artifacts: JSONValue | None = Field(
        default=None, description="Optional artifacts produced by the task"
    )
...
class OrchestrationResultModel(BaseModel):
    ...
    merged: dict[str, JSONValue] = Field(
        default_factory=dict, description="Optional merged outputs keyed by name"
    )
