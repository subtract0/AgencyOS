...
from shared.type_definitions.result import Err, Ok, Result, ResultStr
...
class EnhancedMemoryStoreResult(MemoryStore):
    ...
    def __init__(
        ...
        logger.info(
            f"EnhancedMemoryStoreResult initialized with embedding provider: {embedding_provider}"
        )
    ...
    def search_result(self, tags: list[str]) -> Result[MemorySearchResult, str]:
        ...
        try:
            ...
            search_result = MemorySearchResult(
                records=memory_records, total_count=len(memory_records), search_query={"tags": tags}
            )
            return Ok(search_result)
        except Exception as e:
            return Err(f"{MemoryStoreError.SEARCH_FAILED}: {str(e)}")

    def semantic_search_result(
        self, query: str, top_k: int = 10, min_similarity: float = 0.5
    ) -> Result[list[dict[str, JSONValue]], str]:
        """
        ...
        if not (0.0 <= min_similarity <= 1.0):
            return Err(
                f"{MemoryStoreError.SEARCH_FAILED}: min_similarity must be between 0.0 and 1.0"
            )
    ...
    def combined_search_result(
        self, tags: list[str] | None = None, query: str | None = None, top_k: int = 10
    ) -> Result[list[dict[str, JSONValue]], str]:
        """
        ...
        if tags and not query:
            return self.search_result(tags).map(
                lambda result: [rec.to_dict() for rec in result.records[:top_k]]
            )
    ...
    def get_all(self) -> MemorySearchResult:
        ...
        return MemorySearchResult(
            records=memory_records, total_count=len(memory_records), search_query={}
        )

    # Private helper methods

    def _convert_to_memory_record(
        self, key: str, content: Any, tags: list[str]
    ) -> ResultStr[dict[str, JSONValue]]:
        """Convert content to memory record format."""
        try:
            metadata = MemoryMetadata(additional={"source": "enhanced_memory_store"})
    ...
    def _add_to_vector_store(
        self, key: str, memory_record: dict[str, JSONValue]
    ) -> ResultStr[None]:
    ...
    def _perform_vector_search(
        self, query: str, memories: list[dict[str, JSONValue]], top_k: int, min_similarity: float
    ) -> Result[list[dict[str, JSONValue]], str]:
    ...
    def _perform_vector_search_on_subset(
...
def create_enhanced_memory_store_result(
    embedding_provider: str = "sentence-transformers",
