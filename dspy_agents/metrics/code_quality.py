...
def code_agent_metric(example: Any, prediction: Any, trace: Optional[Any] = None) -> float:
    """
    ...
    try:
        ...
        if hasattr(example, 'requirements') and hasattr(prediction, 'code_changes'):
            ...
            requirements_met = all(
                any(req.lower() in str(change).lower()
                    for change in prediction.code_changes)
...
def auditor_agent_metric(example: Any, prediction: Any, trace: Optional[Any] = None) -> float:
    """
...
def planner_agent_metric(example: Any, prediction: Any, trace: Optional[Any] = None) -> float:
    """
...
def learning_agent_metric(example: Any, prediction: Any, trace: Optional[Any] = None) -> float:
    """
...
def constitutional_compliance_metric(prediction: Any, trace: Optional[Any] = None) -> float:
    """
...
def overall_agent_metric(example: Any, prediction: Any, agent_type: str, trace: Optional[Any] = None) -> float:
    """
    ...
    logger.info(f"Overall metric for {agent_type}: task={task_score:.2f}, constitutional={constitutional_score:.2f}, overall={overall_score:.2f}")
